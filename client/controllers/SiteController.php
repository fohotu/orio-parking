...
class SiteController extends Controller
{
    ...
    {   
        ...
        $r=  \common\models\Dummy::getEvents(102);  
       // var_dump($r);exit;
        $data = [];

        for($i=1;$i<60;$i++){
            $type = rand(99,999)."ee";
            $number = "Y".$i.rand(11,99)."AA".rand(99,999);
            $date_time = "20.02.2023 18:26:48";
            $company = "rrr";
            $employee = "Ивановна";
        ...
        $provider = new \yii\data\ArrayDataProvider([
            ...
            'sort' => [
                'attributes' => ['type', 'number','date_time','company','employe'],
            ],
        ]);
    ...
    public function actionWriteToHistory()
    {
        ...
        foreach($all as $item){
            $data [] = [
                'car_id' => $item->hasDb,
                'enter_date' => $item->enter ? $item->enter:0,
                'exit_date' => $item->exit ? $item->enter:0,
                'orion_event' => serialize($item),
            ];
        }
        
        Yii::$app->db->insert($data);
    }
